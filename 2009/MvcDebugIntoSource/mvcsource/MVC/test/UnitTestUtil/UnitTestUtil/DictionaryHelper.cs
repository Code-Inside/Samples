namespace System.Web.TestUtil {
    ...
    public class DictionaryHelper<TKey, TValue> {
        ...
        public IList<TKey> SampleKeys {
            get;
            set;
        }

        public IList<TValue> SampleValues {
            get;
            set;
        }

        public bool SkipItemPropertyTest {
            get;
            set;
        }
        ...
        private class Executor {
            ...
            public void TestClear() {
                ...
                // Assert
                Assert.AreEqual(0, testDictionary.Count, "Test dictionary was not cleared correctly.");
            }
            ...
            public void TestCountProperty() {
                ...
                foreach (var entry in MakeKeyValuePairs()) {
                    controlDictionary.Add(entry.Key, entry.Value);
                    testDictionary.Add(entry.Key, entry.Value);
                    Assert.AreEqual(controlDictionary.Count, testDictionary.Count, "Count is not correct.");
                }
            }
            ...
            public void TestContains() {
                ...
                Assert.IsTrue(testDictionary.Contains(shouldBeFound), String.Format("Test dictionary should have contained entry for KVP '{0}'.", shouldBeFound));
                Assert.IsFalse(testDictionary.Contains(shouldNotBeFound), String.Format("Test dictionary should not have contained entry for KVP '{0}'.", shouldNotBeFound));
            }
            ...
            public void TestContainsKey() {
                ...
                // Assert
                Assert.IsTrue(testDictionary.ContainsKey(NonConflictingKeys[0]), String.Format("Test dictionary should have contained entry for key '{0}'.", NonConflictingKeys[0]));
                Assert.IsFalse(testDictionary.ContainsKey(ExcludedKey), String.Format("Test dictionary should not have contained entry for key '{0}'.", ExcludedKey));
            }
            ...
            public void TestCopyTo() {
                ...
                for (int i = 0; i < 2; i++) {
                    ...
                    Assert.AreEqual(defaultValue, entry, "Original array entries should have remain unchanged.");
                }
                for (int i = 2; i < testKvps.Length; i++) {
                    var entry = testKvps[i];
                    Assert.IsTrue(controlDictionary.Contains(entry), String.Format("The value '{0}' wasn't present in the control dictionary.", entry));
                    controlDictionary.Remove(entry);
                }

                Assert.AreEqual(0, controlDictionary.Count, "Not all elements were enumerated.");
            }
            ...
            public void TestGetEnumerator() {
                ...
                foreach (object entry in testDictionaryAsEnumerable) {
                    Assert.IsInstanceOfType(entry, typeof(KeyValuePair<TKey, TValue>), "Entry was not of correct type.");
                    var kvp = (KeyValuePair<TKey, TValue>)entry;
            ...
            public void TestIsReadOnlyProperty() {
                ...
                // Act & assert
                Assert.IsFalse(testDictionary.IsReadOnly, "Dictionary should not be read only.");
            }
            ...
            public void TestItemProperty() {
                ...
                Assert.AreEqual(shouldBeFound.Value, value, "Value from get_Item() was not correct.");
                ...
                if (ThrowOnKeyNotFound) {
                    ExceptionHelper.ExpectException<KeyNotFoundException>(
                        ...
                        }, true /* allowDerivedExceptions */);
                }
                else {
                    TValue valueNotFound = testDictionary[shouldNotBeFound.Key];
                    Assert.AreEqual(default(TValue), valueNotFound, "get_Item() should not throw on key not found.");
                }
            }
            ...
            public void TestKeysProperty() {
                ...
                // Assert
                Assert.IsTrue(controlKeys.SetEquals(testKeys), "Control dictionary and test dictionary key sets were not equal.");
            }
            ...
            public void TestRemove1() {
                ...
                // Assert
                Assert.IsTrue(removalSuccess, "Remove() should return true if the key was removed.");
                Assert.IsFalse(removalFailure, "Remove() should return false if the key was not removed.");
            ...
            public void TestTryGetValue() {
                ...
                // Assert
                Assert.IsTrue(returned1, String.Format("The entry '{0}' should have been found.", shouldBeFound));
                Assert.AreEqual(shouldBeFound.Value, value1, "The returned value was invalid.");
                Assert.IsFalse(returned2, String.Format("The entry '{0}' should not have been found.", shouldNotBeFound));
                Assert.AreEqual(default(TValue), value2, "The returned value should have been default(TValue).");
            }
            ...
            public void TestValuesProperty() {
                ...
                foreach (var entry in controlValues) {
                    Assert.IsTrue(testValues.Contains(entry), String.Format("Test dictionary did not contain value '{0}'.", entry));
                }
                foreach (var entry in testValues) {
                    Assert.IsTrue(controlValues.Contains(entry), String.Format("Control dictionary did not contain value '{0}'.", entry));
                }
            }
            ...
            private void VerifyDictionaryEntriesEqual(Dictionary<TKey, TValue> controlDictionary, IDictionary<TKey, TValue> testDictionary) {
                Assert.AreEqual(controlDictionary.Count, testDictionary.Count, "Test dictionary count did not match control dictionary count.");
                ...
                foreach (var entry in testDictionary) {
                    var key = entry.Key;
                    Assert.IsTrue(clonedControlDictionary.ContainsKey(entry.Key), String.Format("Control dictionary did not contain key '{0}'.", key));
